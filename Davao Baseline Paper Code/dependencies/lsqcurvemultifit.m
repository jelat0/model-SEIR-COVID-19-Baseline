function [beta,resnorm,residual,exitflag,output,lambda,jacobian] = ...
    lsqcurvemultifit(x_cell, y_cell, mdl_cell, p0, lb, ub, options,...
    idxparams,paramset,pop)
...
% Author: Yonatan Golan 2016, Based on Chen Avinadav's function nlinmultifit.
	...
	for ii = 1:num_curves
		...
		mdl_vec = [mdl_vec, sprintf('mdl_cell{%d}(p,t(%d:%d),idx,pars,popn), ',...
            ii, mdl_ind1, mdl_ind2)];
    ...
    if nargin < 7
        [beta,resnorm,residual,exitflag,output,lambda,jacobian] = ...
            lsqcurvefit(mdl_vec,p0,x_vec,y_vec,lb,ub);
	else
		[beta,resnorm,residual,exitflag,output,lambda,jacobian] = ...
            lsqcurvefit(mdl_vec,p0,x_vec,y_vec,lb,ub,options,idxparams,paramset,pop);
    end
end